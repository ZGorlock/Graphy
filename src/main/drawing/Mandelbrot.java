...
public class Mandelbrot extends Drawing {
    ...
    public static void main(String[] args) {
        Environment2D environment = new Environment2D((int) SCREEN_SIZE.getX(), (int) SCREEN_SIZE.getY());
        ...
        environment.frame.getContentPane().getComponent(0).addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (doScaling) {
                    ...
                    int newScale = 6 - size.precision() + size.scale();
                    size = size.setScale(newScale, RoundingMode.HALF_UP);
    ...
    /**
     * Loads the available points of interest.
     */
    private static void loadPointsOfInterest() {
        if (!POINTS_OF_INTEREST.isEmpty()) {
            return;
        }
    ...
    class Palette {
        ...
        public int getColor(int index) {
            ...
            palette[index] = 0xff000000 + ((int) color.getX() << 16) + ((int) color.getY() << 8) + (int) color.getZ();
            return palette[index];
        }
